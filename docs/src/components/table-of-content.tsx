...
import {
  ...
  chakra,
  Text,
  useColorModeValue as getColorModeValue,
  BoxProps,
  ...
  return (
    ...
    >
      <Text
        ...
        fontWeight="bold"
        fontSize="xs"
        color={getColorModeValue('gray.700', 'gray.400')}
        letterSpacing="wide"
      >
        On this page
      </Text>
        ...
        {headings.map(({ id, text, level }) => (
          <ListItem key={id} title={text} ml={level === 'h3' ? '4' : undefined}>
            <chakra.a
              ...
              color={getColorModeValue('gray.600', 'gray.400')}
              _hover={{
                color: getColorModeValue('gray.900', 'gray.600'),
              }}
            >
              {text}
            </chakra.a>
